...
export function createColorStore(
    ...
    function setColor(color: string, text?: string) {
        ...
        _set({
            ...
            text:
                text ??
                (_tinyColor.isDark() ? textColor.dark : textColor.light),
            cssProp,
            hex: color,
        });
    }
...
export function createThemeStore<T extends SerializedTheme>(baseTheme: T) {
    ...
    function serialize(): SerializedTheme {
        const colors = Object.fromEntries(
            Array.from(get({ subscribe }).colors.entries()).map(([k, v]) => [
                k,
                v.serialize(),
            ]),
        );
    ...
    function toArray(): SerializedColorStore[] {
        return Array.from(get({ subscribe }).colors.values()).map((c) =>
            c.serialize(),
        );
    }
    function loadFrom(serializedTheme: SerializedTheme) {
        const _theme = cloneDeep(serializedTheme) as SerializedTheme;
        const entries = Object.entries(_theme.colors).map(
            ([k, v]) =>
                [k, createColorStore(v.name, v.hex, v.cssProp, v.text)] as [
                    string,
                    ColorStore,
                ],
        );
...
export type ThemeStore<T extends SerializedTheme> = ReturnType<
    typeof createThemeStore<T>
>;
...
export interface ThemePersistence {
    ...
    load: () =>
        | Promise<Partial<SerializedThemeStorage>>
        | Partial<SerializedThemeStorage>;
    addTheme: (
        theme: SerializedTheme,
        storage: SerializedThemeStorage,
    ) => Promise<void> | void;
    removeTheme: (
        theme: SerializedTheme,
        storage: SerializedThemeStorage,
...
export function createThemeStorage<T extends SerializedTheme>(
    _persistence: ThemePersistence,
    baseTheme: T,
    ...rest: T[]
) {
...
export class LocalStorageThemePersistence implements ThemePersistence {
    ...
    load() {
        const themes = JSON.parse(
            localStorage.getItem(this.lsThemesKey) ?? 'null',
        ) as SerializedTheme[] | null;
        const meta = JSON.parse(
            localStorage.getItem(this.lsMetaKey) ?? 'null',
        ) as ThemeStorageMeta | null;
        return {
...
export function createDerivedThemeColors<T extends SerializedTheme>(
    theme: ThemeStore<T>,
) {
    return derived(theme, ($theme) => theme.toArray());
}
